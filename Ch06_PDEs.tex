...
\begin{problem}
    ...
    \[ \pdd{u}{t} = k \pdd{u}{x}. \]
    What does the parameter $k$ control in terms of the physics of the problem?
\end{problem}
...
\begin{thm}[Stability of Finite Differences for the Heat Equation]
    Consider the 1D Heat Equation $u_t = \kappa u_{xx}$.  In the finite difference scheme for the 1D heat equation
    \[ U_i^{n+1} = U_i^n + \frac{\kappa \Delta t}{\Delta x^2} \left( U_{i+1}^n - 2 U_i^n +
    U_{i-1}^n\right) \]
    the solution will be stable for
    \[ \frac{\kappa \Delta t}{\Delta x^2} < \underline{\hspace{1in}} \]
\end{thm}
\begin{proof}
