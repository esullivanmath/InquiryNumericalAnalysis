...
\begin{problem}
    ...
    the derivative.\\
    \verb|function [new_x,dfdx] = FirstDerivFirstOrder(f, xmin, xmax, num_interior_pts)| 
\end{problem}
...
your code from the previous problem and make sure that you can plot \verb|new_x| vs
\verb|dfdx| without having to change their size.  
...
\begin{problem}
    ...
    \begin{verbatim}
    df = @(x) ... % code for the exact derivative
    subplot(1,2,2)
...
\begin{problem}
    ...
    \verb|function [new_x,dfdx] = FirstDerivSecondOrder(f, xmin, xmax, num_interior_pts)|
\end{problem}
...
\begin{problem}
    ...
    interior points used for approximation within the domain. Your function should output
    the x values and y values associated with the derivative.\\
    \verb|function [new_x,ddfdxx] = SecondDerivSecondOrder(f, xmin, xmax, num_interior_pts)|
\end{problem}
...
\begin{problem}
    ...
    \verb|function Area=MyRiemannSum(f, a, b, num_subintervals, type)| \\
...
\begin{problem}\label{prob:first_deriv_data}
    ...
    underlying function. \\
    \verb|function [new_x,dydx] = FirstDeriveFromData(ExcelFileName,Xrange,Yrange)| \\
    In the function all of the inputs are strings.  For example \\
    \verb|FirstDerivativeFromData('MyFunctionData.xlsx','A2:A101','B2:B101')|\\
...
\begin{problem}
    ...
    underlying function. \\
    \verb|function [new_x,dydx] = SecondDeriveFromData(ExcelFileName,Xrange,Yrange)| \\
    In the function all of the inputs are strings.  For example \\
    \verb|SecondDerivativeFromData('MyFunctionData.xlsx','A2:A101','B2:B101')|\\
    Create a test Excel file and a test script that have graphical output showing that
    your MATLAB function is finding the correct derivative.
\end{problem}
...
\begin{problem}\label{prob:trap_from_data}
    ...
    \verb|function Area = TrapezoidFromData(ExcelFilename, Xlist, Ylist)|\\
    In the function all of the inputs are strings.  For example \\
    \verb|Area = TrapezoidFromData('MyFunctionData.xlsx','A2:A101','B2:B101')|\\
...
\begin{problem}
    ...
    To get the {\it exact} answer for these plots use the MATLAB \verb|quad| command.
