...
\begin{problem}[Order of Convergence]
    A root-finding method has a convergence rate of order $M$ if the ratio 
    \[ \frac{|x_{k+1} - x_*|}{|x_k - x_*|^M} \]
    is roughly constant for each iteration $k$.  In this ration, $x_*$ is the exact root, $x_k$ is
    ...
    $|x_{k}-x_*|$ the ``old'' error since it is the error in the method at the old step.
    We will call the value $|x_{k+1}-x_*|$ the ``new'' error since it is the error in the
    method at the new step.
    \begin{enumerate}
        \item[(a)] If we consider the equation 
            \[ C =   \frac{|x_{k+1} - x_*|}{|x_k - x_*|^M} \]
            and take the logarithm (base 10) of both sides then we get
            \[ \log(C) = \log \left( |x_{k+1} - x_*| \right) -
            M \log\left( |x_{k} - x_*| \right). \]
            Solving for $\log(|x_{k+1} - x_*|)$ we get
            \[ \log\left( |x_{k+1} - x_*| \right) = \underline{\hspace{1in}} +
